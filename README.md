...
[![PyPI](https://img.shields.io/pypi/v/mcp-safety)](https://pypi.org/project/mcp-safety/)
...
```bash
pip install mcp-safety

# for scan-live (talks the real MCP protocol to a running server):
pip install "mcp-safety[dynamic]"
```

The PyPI package is `mcp-safety`; the command it installs is `mcp-sentinel`.
